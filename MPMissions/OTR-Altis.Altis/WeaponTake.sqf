0 = _this spawn {
if(isServer) then { 
_myhuman = if (typeName (_this select 0) == "ARRAY") then {
...
// Add a null check
if (isNil "_myhuman" || {isNull _myhuman}) exitWith {
	diag_log "weaponTake.sqf: Invalid unit parameter";
};
_ifItemArifle = -1;
_ifItemSrifle = -1;
_ifItemhgunP = -1;
