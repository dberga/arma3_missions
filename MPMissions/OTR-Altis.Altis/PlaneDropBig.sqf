...
_requireddist = worldSize / 4;
lastPointPos = getPos _myDropPlane;
...
if (_PlayersDropCount < 1) exitWith {};
 ...
 if(_role isEqualTo 'cargo' && (!isPlayer _x) && ((side _x) == sideEnemy) && {isDrop >randomFloorpl1}) then 
 {
 sleep 0.3;	 
 0 = [_x] spawn {
  _x = _this select 0;
...
_dirDir=0; 
  ...
  _posxh set [0,(_posxh select 0)+2500];
  ...
  _posxh = _posxhm;
  _posxh set [1,(_posxh select 1)+2500];
