...
_teleportObject addAction [  
    "<t color='#6C3BAA'>HUNT CIVILIANS</t>",  
    {  
        ...
        {  
            ...
            if (_varName != "" && {(_varName find "Civilian") != -1}) then {  
                _civilianInfo = [  
                    _varName,  
                    _x,  
                ...
                _civilianList pushBack _civilianInfo;  
            };  
        } forEach allUnits;  
          
        if (count _civilianList == 0) exitWith {  
            ["No civilians found", "PLAIN DOWN", 1] remoteExec ["cutText", _caller];  
        };  
        ...
        _title ctrlSetText "TELEPORT TO CIVILIAN";  
        ...
        _infoText ctrlSetText format["Found %1 civilians", count _civilianList];  
        ...
        {  
            _x params ["_varName", "_unit", "_type", "_pos", "_actualName", "_vehicle"];  
              
            private _distance = round (_caller distance _unit);  
            private _status = if (alive _unit) then {"Alive"} else {"Dead"};  
              
            _nearestLocations = nearestLocations [_pos, ["NameCityCapital","NameCity","NameVillage","NameLocal","Hill"], 2000];  
            ...
            private _index = _listBox lbAdd format["%1%2 %3 - %4 - %5m - Near %6", _actualName, _vehicleSuffix, _type, _status, _distance, _locationName];  
            ...
            if (!alive _unit) then {  
                ...
                if (_isAirOrWater) then {  
                    ...
                    if (isPlayer _unit) then {  
                        _listBox lbSetColor [_index, [0, 1, 0, 0.7]];  
                    } else {  
        ...
        _teleportBtn ctrlSetText "TELEPORT TO CIVILIAN";  
        ...
        _teleportBtn ctrlAddEventHandler ["ButtonClick", {  
            ...
            if (_selectedIndex != -1) then {  
              ...
              _x setVariable ["playerTimeout", true];  
                        ...
                        private _infohint = "Teleported near civilian location";  
                    ...
                    ["Selected civilian is dead or no longer exists", "PLAIN DOWN", 1] remoteExec ["cutText", player];  
                };  
            } else {  
                ["Please select a civilian first", "PLAIN DOWN", 1] remoteExec ["cutText", player];  
            };  
        }];  
