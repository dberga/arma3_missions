...
firstTimerRun = true;
myplayers =0;
LastPosition = [0,0,0];//position EndTr;
IsSpawnLoaded = false;
...
_buildings = _LStrigger nearObjects ["House", 30];
{
    _lootPlaces = _x buildingPos -1;   
    if (count _lootPlaces > 0) then {
        _lootPositions = _lootPlaces call BIS_fnc_arrayShuffle;
        _maxLootPerBuilding = 2;
        
        for "_i" from 0 to (_maxLootPerBuilding - 1) do {
            if (_i < count _lootPositions) then {
                _lootPos = _lootPositions select _i;
                _spawnLoot = random 1 > 0.7;
                if (_spawnLoot) then {
                    0 = [_lootPos] call myfuncLS;
                };
            };
        };   
    };  
...
createLootTriggers = {
if (isServer) then {
_building_terrain = ["BUILDING","BUNKER","BUSH","BUSSTOP","CHAPEL","CHURCH","CROSS","TRIANGLE","FORTRESS","FOUNTAIN","FUELSTATION","HOSPITAL","HOUSE","LIGHTHOUSE","MAIN ROAD","POWER LINES","POWERSOLAR","POWERWAVE","POWERWIND","QUAY","RUIN","SHIPWRECK","TOURISM","TRANSMITTER","VIEW-TOWER","WATERTOWER"];
//_building_terrain = ["BUILDING","BUNKER","BUSH","BUSSTOP","CHAPEL","CHURCH","CROSS","FENCE","FOREST BORDER","FOREST SQUARE","FOREST","TRIANGLE","FORTRESS","FOUNTAIN","FUELSTATION","HIDE","HOSPITAL","HOUSE","LIGHTHOUSE","MAIN ROAD","POWER LINES","POWERSOLAR","POWERWAVE","POWERWIND","QUAY","RAILWAY","ROAD","ROCK","ROCKS","RUIN","SHIPWRECK","SMALL TREE","STACK","TOURISM","TRACK","TRAIL","TRANSMITTER","TREE","VIEW-TOWER","WALL","WATERTOWER"];
...
{   
    ...
    if(_amishouldSpawn == 0) then {
        ...
        _lootTrigger setTriggerActivation ["ANYPLAYER", "PRESENT", false];
        _lootTrigger setTriggerStatements
        [
            "this",
            "0=[thisTrigger] spawn {params ['_trigger']; [_trigger] call LootSpawnRandomizer; sleep 7200; _trigger setTriggerActivation ['ANYPLAYER', 'PRESENT', false];};",
            ""
        ];
...
//LSgr > 0
//_lootTrigger setTriggerActivation ["NONE", "NONE", false];

createCarTriggers = {
if (isServer) then {
//_road_terrain = ["BUILDING","BUNKER","BUSH","BUSSTOP","CHAPEL","CHURCH","CROSS","TRIANGLE","FORTRESS","FOUNTAIN","FUELSTATION","HOSPITAL","HOUSE","LIGHTHOUSE","MAIN ROAD","POWER LINES","POWERSOLAR","POWERWAVE","POWERWIND","QUAY","RUIN","SHIPWRECK","TOURISM","TRANSMITTER","VIEW-TOWER","WATERTOWER"];
_road_terrain=["FORTRESS","BUILDING","CHAPEL","CHURCH","CROSS","FUELSTATION","HOSPITAL","LIGHTHOUSE","POWERSOLAR","POWERWAVE","MAIN ROAD"];
...
{   
    ...
    if (VehicleMarkers == 1) then {
        createMarker [_markerName, getPos _x];
        _markerName setMarkerType "respawn_motor";
    ...
    _carTrigger setTriggerActivation ["ANYPLAYER", "PRESENT", false];
	_carTrigger setTriggerStatements
	[
        "this",
        "0=[thisTrigger] spawn {params ['_trigger']; [_trigger] call myfuncCS; sleep 3600; _trigger setTriggerActivation ['ANYPLAYER', 'PRESENT', false];};",
        ""
	];
...
//"(LSgr == 1) || (LSgr == 3)",
//"LSgr > 0",
//_carTrigger setTriggerActivation ["NONE", "NONE", false];
...
AreasArray = [RoundArea,RoundArea*(-log (1-0.9)),RoundArea*(-log (1-0.8)),RoundArea*(-log (1-0.7)),RoundArea*(-log (1-0.6)),RoundArea*(-log (1-0.5)),RoundArea*(-log (1-0.4)),RoundArea*(-log (1-0.3)),RoundArea*(-log (1-0.2)),RoundArea*(-log (1-0.1)),RoundArea*(-log (1-0.05))];
...
_infohint = format ["THE ZONE WILL BE RESTRICTED IN %1 SEC",_SecToShow]; 
...
_infohint = format ["THE PLAY AREA HAS BEEN RESTRICTED"]; [1,[_infohint, "PLAIN DOWN", 0.7]] remoteExec ["cutText"]; 
}
else {
_infohint = format ["THE PLAY AREA IS STABLE NOW. KILL THEM ALL"]; [1,[_infohint, "PLAIN DOWN", 0.7]] remoteExec ["cutText"];
};
};
};
}; 
